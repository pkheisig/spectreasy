...
.residual_detector_channel_order <- function(detectors) {
    ...
    prefix <- sub("^(UV|YG|V|B|Y|G|R)([0-9]+).*$", "\\1", key, perl = TRUE)
    has_prefix <- grepl("^(UV|YG|V|B|Y|G|R)[0-9]+", key, perl = TRUE)
    prefix[!has_prefix] <- ""

    channel <- suppressWarnings(as.integer(sub("^(UV|YG|V|B|Y|G|R)([0-9]+).*$", "\\2", key, perl = TRUE)))
    missing_channel <- !is.finite(channel)
    channel[missing_channel] <- suppressWarnings(as.integer(sub("^[^0-9]*([0-9]+).*$", "\\1", key[missing_channel], perl = TRUE)))
