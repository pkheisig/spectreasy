...
.normalize_control_validation_df <- function(control_df) {
    ...
    df$is.dead <- toupper(.control_validation_as_chr(df$is.dead))
    df$is.dead[!(df$is.dead %in% c("", "TRUE", "T", "1", "YES", "Y"))] <- ""
    df$is.dead[df$is.dead %in% c("T", "1", "YES", "Y")] <- "TRUE"
    df
}
...
.validate_control_validation_rows <- function(df, require_channels = TRUE) {
    ...
    if ("is.dead" %in% colnames(df)) {
        is_af <- is_af | .is_dead_control_row(df$is.dead)
    }
