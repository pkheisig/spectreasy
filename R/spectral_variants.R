...
.spectral_variant_grid_dims <- function(n_nodes) {
...
.cluster_spectral_variant_shapes <- function(shape_mat, n_nodes = 16L, seed = NULL) {
    ...
    if (requireNamespace("FlowSOM", quietly = TRUE)) {
        dims <- .spectral_variant_grid_dims(n_eff)
        centers <- tryCatch(
            {
                som <- FlowSOM::SOM(
                    shape_mat,
...
.learn_spectral_variant_library <- function(M,
                                            enabled = TRUE,
                                            som_nodes = 16L,
                                            cosine_threshold = 0.98,
                                            max_variants = 8L,
    ...
    if (!is.list(reference_positive_events) || length(reference_positive_events) == 0L) {
        ...
        if (isTRUE(warn)) {
            warning(
                "Spectral variants require AutoSpectral SCC positive events stored on the reference matrix. ",
    ...
    for (fluor in fluorophores) {
        stored_events <- if (fluor %in% names(reference_positive_events)) reference_positive_events[[fluor]] else NULL
