...
## Reproducible toy QC example

This small synthetic example runs end-to-end without FCS files and demonstrates
...
colnames(M_toy) <- c("B1-A", "YG1-A", "R1-A")

results_toy <- data.frame(
  FITC = rnorm(200, 0, 0.3),
  PE = rnorm(200, 0, 0.4),
  AF = rnorm(200, 0, 0.2),
  File = rep(c("SampleA", "SampleB"), each = 100),
  check.names = FALSE
)

toy_report <- tempfile(fileext = ".pdf")
generate_qc_report(results_toy, M_toy, output_file = toy_report)
